...
def main(
    ...
    ssl_keyfile: Annotated[str, typer.Option(help="Path to the SSL keyfile for enabling https or wss connections. For Certbot generated SSL certs, this is the `privkey.pem` file.")] = None,
    ssl_certfile: Annotated[str, typer.Option(help="Path to the SSL certfile for enabling https or wss connections. For Certbot generated SSL certs, this is the `fullchain.pem` file.")] = None
):
...
def create_app(client_token: str) -> FastAPI:
    ...
    async def handle_websocket(
        ...
        if websocket.headers["PIPEGATE_CLIENT_TOKEN"] != client_token:
           raise HTTPException(
            status_code=403, detail="Client token missing or invalid"
        )
